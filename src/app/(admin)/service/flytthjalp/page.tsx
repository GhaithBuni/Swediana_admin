// app/(dashboard)/admin/moving/page.tsx
"use client";
...
// Mirror IMovingBooking shape you shared
type Address = {
  postcode: string;
...
export default function MovingPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await api.getBookings(token); // <— moving endpoint
  ...
  const displayNumber = (b: MovingBooking) =>
    b.bookingNumber ?? b._id?.slice(-6).toUpperCase();

  const displayName = (b: MovingBooking) => b.name ?? "—";

  function StatusBadge({ status }: { status?: MovingBooking["status"] }) {
  ...
  function onViewDetails(id: string) {
    // adjust if your detail page lives elsewhere (e.g. /admin/moving/[id])
    router.push(`/service/flytthjalp/${id}`);
  }
  ...
  return (
    <Card>
      <CardHeader className="flex items-center justify-between gap-2">
        <div>
          <CardTitle>Flytt – Bokningar</CardTitle>
          <CardDescription>
                ...
                {bookings.map((b) => {
                  ...
                  return (
                    <TableRow key={id}>
                      ...
                      <TableCell>{formatDateOnly(b.date)}</TableCell>
                      <TableCell>
