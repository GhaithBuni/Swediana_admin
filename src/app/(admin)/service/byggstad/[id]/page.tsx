...
export default function ByggDetailPage({
  params,
}: {
  ...
  const [booking, setBooking] = useState<ByggBooking | null>(null);
  ...
  async function onSave() {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/bygg/${booking._id}`, {
        method: "PATCH",
        headers: {
      ...
      const updated: ByggBooking = data?.booking ?? data;
      setBooking(updated);
  ...
  async function onSendConfirmation() {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/bygg/send-confirmation/${booking._id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
                  ...
                  onChange={(e) =>
                    setEdit((s) =>
                      s
                        ? {
                            ...s,
                            status: e.target.value as ByggBooking["status"],
                          }
                        : s
                    )
                  }
