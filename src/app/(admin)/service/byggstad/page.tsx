...
export default function ByggPage() {
  ...
  const [bookings, setBookings] = useState<ByggBooking[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await api.getBookingsBygg(token);
  ...
  const formatWhen = (b: ByggBooking) =>
    b.date ? formatDateOnly(b.date) : "—";

  const displayNumber = (b: ByggBooking) =>
    b.bookingNumber ?? b.ref ?? b._id?.slice(-6).toUpperCase();

  const displayName = (b: ByggBooking) =>
    b.name ?? b.customerName ?? b.customer ?? "—";

  function StatusBadge({ status }: { status?: ByggBooking["status"] }) {
  ...
  async function onDelete(id: string) {
    ...
    try {
      const token = authStorage.getToken?.();
      const res = await fetch(`${API_URL}/bygg/${id}`, {
        method: "DELETE",
  ...
  async function onSendConfirmation(id: string) {
    ...
    try {
      const token = authStorage.getToken?.();
      const res = await fetch(`${API_URL}/bygg/send-confirmation/${id}`, {
        method: "POST",
        headers: {
  ...
  function onViewDetails(id: string) {
    router.push(`/service/byggstad/${id}`);
  }
